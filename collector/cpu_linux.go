...
func (c *cpuCollector) updateStat(ch chan<- prometheus.Metric) error {
	...
	//	//ch <- prometheus.NewMetricWithTimestamp( ,prometheus.MustNewConstMetric(c.cpu, prometheus.CounterValue, cpuStat.User, cpuNum, "user"))
	//
	...
	ch <- prometheus.MustNewConstMetric(c.cpu, prometheus.CounterValue, cpuNum, "tt1", "vNum")
...
func (c *cpuCollector) updateCPUTotal(n procfs.CPUStat) {
	c.cpuStatsMutex.Lock()
	defer c.cpuStatsMutex.Unlock()
	if (c.cpuTotal.Idle - n.Idle) >= jumpBackSeconds {
		level.Debug(c.logger).Log("msg", jumpBackDebugMessage, "cpu", 1, "old_value", c.cpuTotal.Idle, "new_value", n.Idle)
		c.cpuTotal = procfs.CPUStat{}
	}
	if n.System >= c.cpuTotal.System {
		c.cpuTotal.System = n.System
	} else {
		level.Debug(c.logger).Log("msg", "CPU System counter jumped backwards", "cpu", 1, "old_value", c.cpuTotal.System, "new_value", n.System)
	}
	if n.User >= c.cpuTotal.User {
		c.cpuTotal.User = n.User
	} else {
		level.Debug(c.logger).Log("msg", "CPU System counter jumped backwards", "cpu", 1, "old_value", c.cpuTotal.User, "new_value", n.User)
	}
	if n.Idle >= c.cpuTotal.Idle {
		c.cpuTotal.Idle = n.Idle
	} else {
